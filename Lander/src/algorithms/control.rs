    ...
    fn set_flight_phase(&mut self, phase: FlightPhase) {
        ...
        if !self.manual_weights {
            match phase {
                ...
                FlightPhase::Descent => {
                    self.q = Array2::<f64>::from_diag(&Array1::from(vec![
                        3073.352600769099, 3073.352600769099, 139710.1276331588,
                        973403.4781586826, 973403.4781586826, 0.0, 0.0,
                        12648.559309745955, 12648.559309745955, 406664.647223561,
                        9937.546053248767, 9937.546053248767, 1569.6207019130236
                    ]));
                    self.r = Array2::<f64>::from_diag(&Array1::from(vec![233.0875405092546, 233.0875405092546, 0.18917272513838274]));
                    self.qn = Array2::<f64>::from_diag(&Array1::from(vec![
                        123641.17887946166, 123641.17887946166, 7848929.101756228,
                        9322794.722398248, 9322794.722398248, 0.0, 0.0,
                        167138.56674172482, 167138.56674172482, 740365.6892377232,
                        20736.248327915644, 20736.248327915644, 16745.98404007677
                    ]));
                }
                _ => {}
            }
