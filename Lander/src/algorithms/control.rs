    ...
    fn set_flight_phase(&mut self, phase: FlightPhase) {
        ...
        if !self.manual_weights {
            match phase {
                ...
                FlightPhase::Descent => {
                    self.q = Array2::<f64>::from_diag(&Array1::from(vec![
                        63.50988949825596, 63.50988949825596, 946.260417689045,
                        742900.8388804018, 742900.8388804018, 0.0, 0.0,
                        603.5557178399907, 603.5557178399907, 29000.196886678063,
                        11491.26826743796, 11491.26826743796, 622.1444989888226
                    ]));
                    self.r = Array2::<f64>::from_diag(&Array1::from(vec![36.07731488312801, 36.07731488312801, 0.01225425568348351]));
                    self.qn = Array2::<f64>::from_diag(&Array1::from(vec![
                        37.098008383796135, 37.098008383796135, 2900.6586839033803,
                        2146796.4902601726, 2146796.4902601726, 0.0, 0.0,
                        266.4773620259802, 266.4773620259802, 21339.235884421676,
                        10928.714433495239, 10928.714433495239, 16158.617448578461
                    ]));
                }
                _ => {}
            }
